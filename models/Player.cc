...
Player::Player(const Row &r, const ssize_t indexOffset) noexcept
{
    if(indexOffset < 0)
    {
        ...
        if(!r["clan"].isNull())
        {
            clan_=std::make_shared<int32_t>(r["clan"].as<int32_t>());
        }
    ...
    else
    {
        ...
        if(!r[index].isNull())
        {
            clan_=std::make_shared<int32_t>(r[index].as<int32_t>());
        }
        index = offset + 8;
        if(!r[index].isNull())
        {
...
Player::Player(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false)
{
    ...
    if(!pMasqueradingVector[7].empty() && pJson.isMember(pMasqueradingVector[7]))
    {
        ...
        {
            clan_=std::make_shared<int32_t>((int32_t)pJson[pMasqueradingVector[7]].asInt64());
        }
    }
...
Player::Player(const Json::Value &pJson) noexcept(false)
{
    ...
    if(pJson.isMember("clan"))
    {
        ...
        {
            clan_=std::make_shared<int32_t>((int32_t)pJson["clan"].asInt64());
        }
    }
...
void Player::setUsernameToNull() noexcept
{
    username_.reset();
    dirtyFlag_[1] = true;
}
...
const int32_t &Player::getValueOfClan() const noexcept
{
    const static int32_t defaultValue = int32_t();
    if(clan_)
        return *clan_;
...
const std::shared_ptr<int32_t> &Player::getClan() const noexcept
{
    return clan_;
}
void Player::setClan(const int32_t &pClan) noexcept
{
    clan_ = std::make_shared<int32_t>(pClan);
    dirtyFlag_[7] = true;
}
...
const std::string &Player::getValueOfPasswordHash() const noexcept
{
    const static std::string defaultValue = std::string();
    if(passwordHash_)
...
const std::shared_ptr<std::string> &Player::getPasswordHash() const noexcept
{
    return passwordHash_;
}
void Player::setPasswordHash(const std::string &pPasswordHash) noexcept
{
    passwordHash_ = std::make_shared<std::string>(pPasswordHash);
    dirtyFlag_[9] = true;
}
void Player::setPasswordHash(std::string &&pPasswordHash) noexcept
{
    passwordHash_ = std::make_shared<std::string>(std::move(pPasswordHash));
    dirtyFlag_[9] = true;
}
void Player::setPasswordHashToNull() noexcept
{
    passwordHash_.reset();
    dirtyFlag_[9] = true;
}
